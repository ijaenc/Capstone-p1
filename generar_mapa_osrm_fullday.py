...
def main():
    ...
    for (ventana, cluster_id, ruta_id), df_ruta in grupos_ruta:
        ...
        folium.PolyLine(
            ...
            opacity=0.85,
            color=color,
            tooltip=f"{fecha_objetivo} | {ventana} | cluster {cluster_id} | ruta {ruta_id}"
        ).add_to(mapa)
        ...
        for _, row in df_ruta.iterrows():
            ...
            if row["es_deposito"] == 1:
                icono = folium.Icon(color="red", icon="home")
            else:
                icono = folium.Icon(color=color)
    ...
    ruta_html = os.path.join(carpeta_salida, f"mapa_{fecha_objetivo}_AM_PM.html")
    mapa.save(ruta_html)

    pd.DataFrame(resumen).to_csv(
        os.path.join(carpeta_salida, f"resumen_{fecha_objetivo}_AM_PM.csv"),
        index=False,
        encoding="utf-8-sig"
    )
